...
void ActionManager::CsCl(int x, int y, int z, float a) {
    const float acell = a;//std::sqrt(4) / 3;
        // create a vector to store coordinates
        std::vector<float> posX, posY, posZ, result, typePos;
        ...
        posX.push_back(0); posY.push_back(acell); posZ.push_back(acell); typePos.push_back(1);
        posX.push_back(acell); posY.push_back(0); posZ.push_back(acell); typePos.push_back(1);
        ...
        std::ofstream output;
        output.open("CsCl.xyz");
	//output.open("~/ovito/src/gui/actions/CSCL.xyz");
